...
{      
    ...
    Dictionary<int, HashSet<int>> trailHeadCounts = new Dictionary<int, HashSet<int>>();
        ...
        if (newX >= 0 && newX < RowLength && newY >= 0 && newY < ColumnLength){
            ...
            if (c == item.Item + 1){
                ...
                if (c == 9){
                    ...
                    if (!trailHeadCounts.TryGetValue(key, out HashSet<int>? ends))
                        ends = new HashSet<int>();
                    ends.Add( newX * 10000 + newY );
                    Console.WriteLine($"({item.StartX},{item.StartY}-->({newX},{newY}) up to {ends.Count})");
